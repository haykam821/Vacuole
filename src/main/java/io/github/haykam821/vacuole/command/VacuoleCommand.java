...
public final class VacuoleCommand {
	...
	private static int list(CommandContext<ServerCommandSource> context) {
		context.getSource().sendFeedback(new TranslatableText("command.vacuole.treasure.list.header", TreasureType.REGISTRY.keySet().size()), false);

		for (Identifier id : TreasureType.REGISTRY.keySet()) {
			TreasureType<?> treasure = TreasureType.REGISTRY.get(id);
			context.getSource().sendFeedback(new TranslatableText("command.vacuole.treasure.list.entry", treasure.getName()), false);
		}
